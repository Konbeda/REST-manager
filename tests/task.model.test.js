...
describe('Task (schema)', () => {
  it('aceita uma task mínima válida', () => {
    const task = new Task({ title: 'Comprar café' });
  ...
  it('rejeita título com menos de 3 caracteres', () => {
    const err = new Task({ title: 'ab' }).validateSync();
  ...
  it.each(TASK_STATUS)('aceita o status "%s"', (status) => {
    const task = new Task({ title: 'Comprar café', status });
    expect(task.validateSync()).toBeUndefined();
  });

  it('rejeita status fora do enum', () => {
    const err = new Task({ title: 'Comprar café', status: 'pendente' }).validateSync();
