...
describe('DELETE /tasks/:id', () => {
  ...
  it('é idempotente: apagar duas vezes dá 204 nas duas', async () => {
    ...
    const primeira = await request(app).delete(`/tasks/${task._id}`);
    const segunda = await request(app).delete(`/tasks/${task._id}`);
  ...
  it('responde 204 para id que nunca existiu', async () => {
    const res = await request(app).delete(`/tasks/${new mongoose.Types.ObjectId()}`);
  ...
  it('ainda recusa id malformado com 400', async () => {
    const res = await request(app).delete('/tasks/banana');
...
describe('Task apagada some das outras rotas', () => {
  ...
  beforeEach(async () => {
    ...
    await request(app).delete(`/tasks/${apagada._id}`);
  });
  ...
  it('devolve 404 na busca por id', async () => {
    const res = await request(app).get(`/tasks/${apagada._id}`);
